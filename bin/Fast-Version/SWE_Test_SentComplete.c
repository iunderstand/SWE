...
#endif
#ifdef ON_WINDOWS
	unsigned int __stdcall TestModelThread(void *id){
#endif
...
//// Core function for Sentence Completion ////
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
	printf("SWE toolkit: Word vectors for MSR Sentence Completion\n\n");
	printf("See the corresponding testing scripts for usages\n");
	return 0;
	}
	output_file[0] = 0;
	save_vocab_file[0] = 0;
    ...
    printf("SWE Tool For: Microsoft Sentence Completion\n");
	printf("MODEL-Evaluation: Load Word Embedding Models and Testing\n");
	printf("Setting embedding size: %d\n", layer1_size);
	printf("Setting window size: %d\n", window);
	printf("Setting sample value: %f\n", sample);	
	printf("Setting negative num: %d\n", negative);
